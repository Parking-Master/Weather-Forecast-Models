...
async function collect() {
  try {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      let wind = liveWeatherData.observations.map(x => x.wspd || 0).reduce((a, b) => a + b) / liveWeatherData.observations.filter(x => (x !== 0 && x !== null)).length;
      ...
      let pressure = liveWeatherData.observations.map(x => x.pressure || 0).reduce((a, b) => a + b) / liveWeatherData.observations.filter(x => (x !== 0 && x !== null)).length;
      let visibility = liveWeatherData.observations.map(x => x.vis || 0).reduce((a, b) => a + b) / liveWeatherData.observations.filter(x => (x !== 0 && x !== null)).length;
      let rain = liveWeatherData.observations.filter(x => x.precip_hrly > 0).length > 0;

      weatherDataDayList.temp = temp;
      weatherDataDayList.wind = Math.round(wind);
      weatherDataDayList.humidity = humidity.toFixed(1) - 0;
      weatherDataDayList.pressure = pressure.toFixed(2) - 0;
      weatherDataDayList.visibility = visibility;
